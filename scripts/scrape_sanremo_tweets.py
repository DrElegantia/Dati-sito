...
def scrape_day(gdl, browser, account, hashtag, day, outdir):
    """Scrape tweets for one day using gallery-dl."""
    since = day.isoformat()
...
def main():
    parser = argparse.ArgumentParser(description='Scrape Sanremo tweet da @SanremoRai')
    ...
    parser.add_argument('--browser', default='chrome', help='Browser per cookies (default: chrome)')
    ...
    for y in years:
        try:
            r = process_year(y, browser=args.browser, gdl_path=gdl)
            results.append(r)
        except Exception as e:
